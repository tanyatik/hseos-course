...
## Dot-файл
.bashrc -- файл пользовательских настроек командного интерпретатора bash.

Каждый раз, когда пользователь `user` запускает команду bash, bash выполняет команды, записанные в `.bashrc`.

## Source

`source` -- встроенная команда bash. Она считывает и выполняет команды из файла, указанного в качестве ее аргумента.

`.` -- это псевдоним (алиас) команды source.

Один из примеров использования -- повторно зачитать файл настроек `.bashrc` в уже запущенном интерпретаторе:
`. ~/.bashrc`
Или, что то же самое:
`source ~/.bashrc`
...
Для любой команды в bash можно указать псевдоним (alias).
Пример:
```
alias ll=ls -l
ll
...
При выполнении `ll` выполнится `ls -l`. Псевдоним сохраняется в текущем сеансе командного интерпретатора.

<!--
# Домашнее задание

1. Написать shell-скрипт, который выполняет очистку "мусорных" файлов в
